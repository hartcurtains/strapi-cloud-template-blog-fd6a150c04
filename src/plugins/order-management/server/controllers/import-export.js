        ...
        for (let i = 0; i < transformedDataset[productType].length; i++) {
          try {
            ...
            if (schemasWithProductId.includes(productType)) {
              ...
              // Generate a valid slug before entityService persistence.
              if (typeof item.slug !== 'string' || item.slug.trim() === '') {
                const slugSource = item.productId || item.name || 'fabric';
                const generatedSlug = String(slugSource)
                  .normalize('NFKD')
                  .replace(/[\u0300-\u036f]/g, '')
                  .toLowerCase()
                  .replace(/[^a-z0-9]+/g, '-')
                  .replace(/^-+|-+$/g, '');
                item.slug = generatedSlug || 'fabric';
                // Keep the generated slug explicit because entityService requires it.
                console.log(`🔧 Using generated slug for ${productType}: ${item.slug}`);
              }
            }
