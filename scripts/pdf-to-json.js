 ...
 * Converts price-list.pdf to JSON format for import into Strapi
 * 
 * Usage: node scripts/pdf-to-json.js
 * Output: Prints JSON to console (can be copied and pasted)
 */
...
async function main() {
  try {
    console.log('📄 Starting PDF to JSON conversion...\n');
    
    // Find PDF file (look in project root or scripts directory)
    const projectRoot = path.join(__dirname, '..');
    const pdfPath = path.join(projectRoot, 'price-list.pdf');
    
    if (!fs.existsSync(pdfPath)) {
      console.error(`❌ Error: PDF file not found at ${pdfPath}`);
      console.error('Please ensure price-list.pdf is in the project root directory.');
      process.exit(1);
    }
    ...
    console.log(`   - Fabrics: ${parsedData.fabrics.length} items`);
    console.log(`   - Care Instructions: ${parsedData.care_instructions.length} items\n`);
    
    // Output JSON
    console.log('📋 JSON Output (copy this and paste into the import modal):\n');
    console.log('='.repeat(80));
    console.log(JSON.stringify(parsedData, null, 2));
    console.log('='.repeat(80));
    console.log('\n✅ Conversion complete! Copy the JSON above and paste it into the Product Management import modal.');
